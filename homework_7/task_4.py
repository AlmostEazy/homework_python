...
следующие атрибуты: speed, color, name, is_police (булево). А также
методы: go, stop, turn(direction), которые должны сообщать, что машина
поехала, остановилась, повернула (куда). Опишите несколько дочерних
...
class WorkCar(Car):
    ...
    def show_speed(self):
        ...
        if self.speed > 60:
            return f"Speed of {self.name} " \
                   f"is higher than allow for work car"
...
Bugatti = SportCar(100, "White", "Bugatti", False)
KIA = TownCar(30, "Green", "KIA", False)
Hyundai = WorkCar(70, "Yellow", "Hyundai", True)
Audi = PoliceCar(110, "Black",  "Audi", True)
